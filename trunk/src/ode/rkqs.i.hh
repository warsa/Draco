	  ...
	  Function derivs)
{
    ...
    {
        ytemp[i] = y[i] +
            h*(b51*dydx[i] + b52*ak2[i] + b53*ak3[i] + b54*ak4[i]);
    }
    ...
    {
        yout[i] = y[i] +
            h*(c1*dydx[i] + c3*ak3[i] + c4*ak4[i] + c6*ak6[i]);
    }
    for( unsigned i = 0; i < n; i++ )
    {
        yerr[i] =
            h*(dc1*dydx[i] + dc3*ak3[i] + dc4*ak4[i] + dc5*ak5[i]
            + dc6*ak6[i]);
    }
}
          ...
          Function derivs)
{
    ...
    for( ;;)
    {
        ...
        double xnew = x + h;
        if( xnew == x ) throw std::range_error( "stepsize underflow in rkqs" );
    }
    if( errmax > ERRCON ) hnext = SAFETY*h*std::pow( errmax, PGROW );
    else hnext = 5 * h;
    x += (hdid = h);
    for( unsigned i = 0; i < n; i++ ) y[i] = ytemp[i];
}
