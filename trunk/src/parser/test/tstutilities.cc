...
void tstutilities(UnitTest &ut)
{
    ...
    if (left!=J) FAILMSG("test fails");

    left = parse_unit(tokens);
    Unit right = parse_unit(tokens);
    if (left!=right || left!=C) FAILMSG("test fails");

    left = parse_unit(tokens);
    cout << left << endl;
    if (left!=1/s) FAILMSG("test fails");

    left = parse_unit(tokens);
    right = parse_unit(tokens);
    if (left!=right || left!=N) FAILMSG("test fails");

    left = parse_unit(tokens);
    if (left!=J/K) FAILMSG("test fails");

    left = parse_unit(tokens);
    if (left!=J/mol) FAILMSG("test fails");

    left = parse_unit(tokens);
    right = parse_unit(tokens);
    if (left!=right || left!=lm) FAILMSG("test fails");

    left = parse_unit(tokens);
    if (left!=rad/s) FAILMSG("test fails");

    left = parse_unit(tokens);
    if (left!=one) FAILMSG("test fails");
