...
Class_Parse_Table<Quadrature>::Class_Parse_Table()
{
    ...
    if (first_time)
    {
        register_quadrature("gauss legendre",
                            parse_gauss_legendre);

        register_quadrature("level symmetric",
                            parse_level_symmetric);

        register_quadrature("tri cl",
                            parse_tri_cl);

        register_quadrature("square cl",
                            parse_square_cl);

        register_quadrature("product cl",
                            parse_product_cl);

        register_quadrature("double gauss",
                            parse_double_gauss);

        register_quadrature("general octant quadrature",
                            parse_general_octant_quadrature);

        register_quadrature("lobatto",
                            parse_lobatto);
...
{
    tokens.check_semantics(parsed_quadrature_,
