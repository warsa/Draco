...
void tstzbrent( UnitTest & ut )
{
    ...
    if (soft_equiv(zbrent<fpdd>(log_fpdd,
				x1,
				x2,
				100,
				xtol,
    ...
    if (soft_equiv(zbrent(zbrac_test_function, x1, x2, 100, xtol, ftol),
		   1.0))
    {
    ...
    xtol = numeric_limits<double>::epsilon();
    ftol = numeric_limits<double>::epsilon();
    if (soft_equiv(zbrent<fpdd>(expm1_fpdd,
				x1,
				x2,
				100,
				xtol,
