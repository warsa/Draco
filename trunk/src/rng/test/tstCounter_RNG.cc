...
void test_equality(UnitTest &ut)
{
    ...
    if (rng.size() != CBRNG_DATA_SIZE)                                 ITFAILS;
    if (rng.size_bytes() != CBRNG_DATA_SIZE * sizeof(uint64_t))        ITFAILS;
    if (rng != rng)                                                    ITFAILS;
    ...
    if (rng5 == rng4)                                                  ITFAILS;
    if (rng5 != rng5)                                                  ITFAILS;
    ...
    if (rng6 != rng4)                                                  ITFAILS;
    if (rng6 == rng5)                                                  ITFAILS;
    ...
    if ( std::equal(rng6.begin(), rng6.end(), rng2.begin()))           ITFAILS;
    if ( std::equal(rng6.begin(), rng6.end(), rng3.begin()))           ITFAILS;
    if (!std::equal(rng6.begin(), rng6.end(), rng4.begin()))           ITFAILS;
    if ( std::equal(rng6.begin(), rng6.end(), rng5.begin()))           ITFAILS;
...
void test_alias(UnitTest &ut)
{
    ...
    if (rng2 == rng4)                                                  ITFAILS;
    if (rng3 == rng4)                                                  ITFAILS;

    // Create a Counter_RNG_Ref from rng.
    Counter_RNG_Ref ref(rng.ref());

    if (ref.get_num() != rng.get_num())                                ITFAILS;
    ...
    if (ref.is_alias_for(rng4))                                        ITFAILS;
    ...
    if (!std::equal(rng.begin(), rng.end(), data.begin()))             ITFAILS;
    ...
    if (ref2.get_num() != rng.get_num())                               ITFAILS;
    ...
    if (ref2.is_alias_for(rng4))                                       ITFAILS;
    ...
    if (data[0] != 1)                                                  ITFAILS;
    if (data[1] != static_cast<uint64_t>(0x2222) << 32)                ITFAILS;
    if (data[2] != 0x20202020)                                         ITFAILS;
    if (data[3] != 0)                                                  ITFAILS;
    if (soft_equiv(y, x))                                              ITFAILS;

    // Generate a random double from rng3; it should match the one from ref2.
    double z = rng3.ran();

    if (!soft_equiv(z, y))                                             ITFAILS;
...
void test_rollover(UnitTest &ut)
{
    ...
    double y2 = ref.ran();
    if (!soft_equiv(y2, y))                                            ITFAILS;
    if (data[0] != 0xffffffffffffffff)                                 ITFAILS;
    if (data[1] != 1)                                                  ITFAILS;
    if (data[2] != 0xabcd)                                             ITFAILS;
    if (data[3] != 0xef00)                                             ITFAILS;

    double z2 = ref.ran();
    if (!soft_equiv(z2, z))                                            ITFAILS;
    if (data[0] != 0)                                                  ITFAILS;
    if (data[1] != 2)                                                  ITFAILS;
    if (data[2] != 0xabcd)                                             ITFAILS;
    if (data[3] != 0xef00)                                             ITFAILS;
...
void test_spawn(UnitTest &ut)
{
    ...
    // identical to rng.
    if (rng_child1.get_num() != streamnum)                             ITFAILS;
    if (rng.get_num() != streamnum)                                    ITFAILS;
    if (rng_child1 == rng)                                             ITFAILS;

    // Create a reference to rng, and spawn from the reference.
    Counter_RNG_Ref rng_ref(rng.ref());
    if (!rng_ref.is_alias_for(rng))                                    ITFAILS;
    ...
    if (rng_child2.get_num() != streamnum)                             ITFAILS;
    if (rng_child1.get_num() != streamnum)                             ITFAILS;
    if (rng.get_num() != streamnum)                                    ITFAILS;
    if (rng_child2 == rng_child1)                                      ITFAILS;
    if (rng_child2 == rng)                                             ITFAILS;
    if (rng_child1 == rng)                                             ITFAILS;
    ...
    if (rng_grandchild1.get_num() != streamnum)                        ITFAILS;
    if (rng_child2.get_num() != streamnum)                             ITFAILS;
    if (rng_child1.get_num() != streamnum)                             ITFAILS;
    if (rng.get_num() != streamnum)                                    ITFAILS;
    if (rng_grandchild1 == rng_child2)                                 ITFAILS;
    if (rng_grandchild1 == rng_child1)                                 ITFAILS;
    if (rng_grandchild1 == rng)                                        ITFAILS;
    if (rng_child2 == rng_child1)                                      ITFAILS;
    if (rng_child2 == rng)                                             ITFAILS;
    if (rng_child1 == rng)                                             ITFAILS;
    ...
    if (rng_grandchild2.get_num() != streamnum)                        ITFAILS;
    if (rng_grandchild1.get_num() != streamnum)                        ITFAILS;
    if (rng_child2.get_num() != streamnum)                             ITFAILS;
    if (rng_child1.get_num() != streamnum)                             ITFAILS;
    if (rng.get_num() != streamnum)                                    ITFAILS;
    if (rng_grandchild2 == rng_grandchild1)                            ITFAILS;
    if (rng_grandchild2 == rng_child2)                                 ITFAILS;
    if (rng_grandchild2 == rng_child1)                                 ITFAILS;
    if (rng_grandchild2 == rng)                                        ITFAILS;
    if (rng_grandchild1 == rng_child2)                                 ITFAILS;
    if (rng_grandchild1 == rng_child1)                                 ITFAILS;
    if (rng_grandchild1 == rng)                                        ITFAILS;
    if (rng_child2 == rng_child1)                                      ITFAILS;
    ...
    for (gen = 0; gen < timeout; ++gen)
    {
        ...
        // either.
        if (child.get_num() != streamnum)                              ITFAILS;
        if (parent.get_num() != streamnum)                             ITFAILS;
        if (child == parent)                                           ITFAILS;
        if (child == original)                                         ITFAILS;
        if (parent == original)                                        ITFAILS;
        ...
        {
            std::cout << "Found parent among previous generators after "
                      << gen << " generations." << std::endl;
            break;
        }

        if (spawn_id.find(child_id) != spawn_id.end())
        {
            std::cout << "Found child among previous generators after "
                      << gen << " generations." << std::endl;
            break;
        }
    ...
    if (gen != expected_period)                                        ITFAILS;
    ...
    if (!parent_ref.is_alias_for(parent))                              ITFAILS;
    ...
    for (gen = 0; gen < timeout; ++gen)
    {
        ...
        if (grandchild.get_num() != streamnum)                         ITFAILS;
        if (child.get_num() != streamnum)                              ITFAILS;
        if (parent.get_num() != streamnum)                             ITFAILS;
        if (grandchild == child)                                       ITFAILS;
        if (grandchild == parent)                                      ITFAILS;
        if (grandchild == original)                                    ITFAILS;
        ...
        {
            std::cout << "Found grandchild among previous generators after "
                      << gen << " generations." << std::endl;
            break;
        }
...
void test_unique(UnitTest &ut)
{
    ...
    for (unsigned int i = 0; i < expected_period; ++i)
    {
        ...
        if (rng3.get_num()       != rng.get_num())                     ITFAILS;
        if (rng3_child.get_num() != rng.get_num())                     ITFAILS;

        if (rng_ref.get_unique_num()  != rng.get_unique_num())         ITFAILS;
        if (rng3_ref.get_unique_num() != rng3.get_unique_num())        ITFAILS;

        if (ids.find(rng3.get_unique_num())       != ids.end())        ITFAILS;
        if (ids.find(rng3_child.get_unique_num()) != ids.end())        ITFAILS;
