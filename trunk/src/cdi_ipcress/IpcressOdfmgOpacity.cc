...
void IpcressOdfmgOpacity::loadGroupsAndBands(size_t const numBands)
{
    ...
    for (size_t group = 0; group < numGroups; group++)
    {
        ...
        double const groupWidth = groupBoundaries[group + 1] -
        ...
        else
        {
            ...
            if (!soft_equiv(
                    bandBoundaries.begin(), bandBoundaries.end(),
 ...
 */
std::vector< std::vector<double> > IpcressOdfmgOpacity::getOpacity( 
...
{ 
    ...
    {
        tempOpacity[g] = spIpcressDataTable->interpOpac( targetTemperature,
                                                         targetDensity, g );
...
IpcressOdfmgOpacity::getOpacity( 
    std::vector<double> const & targetTemperature,
    double targetDensity ) const
{ 
    std::vector< std::vector< std::vector<double> > >
        opacity( targetTemperature.size() );
