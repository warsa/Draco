...
\milagro)\footnote{Email Rob Lowrie (lowrie@lanl.gov) for
...
individual components.  \draco\ components are primarily written in C++, utilize
...
dependency between components.  In recent months, the \draco\ build system began proving support for Fortran based components and some Fortran code is now provided.  
\draco\ is a software project that is
continually evolving.  Many components that were once actively
developed and used have been retired.  New components are frequently
added to meet today's programming requirements.
...
%it makes it a bit painful to extract information changes since the last release.  
...
% Consider using hotcopy?
% This command (svnadmin hotcopy) makes a safe copy of the active repo
% (including hooks, scripts, etc.) that can then be tarballed, even if
% something happens midstream.   
% http://wordaligned.org/articles/how-to-mirror-a-subversion-repository
% http://svnbook.red-bean.com/en/1.6/svn.ref.svnadmin.c.hotcopy.html
...
%\begin{itemize}
%\item \svn\  major revision numbers could be updated auto-magically
