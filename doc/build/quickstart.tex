...
\url{register.lanl.gov}.  \index{Unix file sharing group}
%%----------------------------------------
\subsection{Important Locations}

Important file and web locations  for \draco\ are listed in Table~\ref{tab:important-locations}.
...
LANL lists you are subscribed to.  Table~\ref{tab:email-lists} below
lists a few other mailing lists that may be useful for CCS--2
developers to subscribe to.

%%----------------------------------------
\subsection{SVN commit notifications}
\index{SVN!commit notifications}
...
LANL listserver.  Table~\ref{tab:email-lists} summarizes the
...
\index{regression reports!Nightly}
...
warnings. \index{dashboard}

%%----------------------------------------
\subsection{Issue Tracking}
\index{issue tracking}

\draco\ officially uses the LANS Team Forge \index{Team Forge} issue tracking system to manage issue,
...
Starting Q4CY12, many key developers have started using CCS-2's Redmine \index{Redmine} project management console to manage issues, bugs, features, releases, etc.  The Redmine console can be accessed by opening a web browser from the Yellow network to \url{coder.lanl.gov/redmine}.  If you need an account please contact someone who is in the \draco\ systems developer group (see Table~\ref{tab:draco_system}).

You are encouraged to browse the list of known bugs/issues of the project you are working on.  
...
recommends that the GNU g++ compiler (v. 4.7+\footnote{gfortran-4.7.0 was the first version that provided complete support for \comp{ISO\_C\_BINDING} used by \draco\ to interface between \lang{Fortran} and \cpp.}) be used for development
...
LANL ASC projects rely on Intel-12.1+ compiler for released code and \draco\ must also work with this tool set.
Code added to the \draco\ repository must also compile without
warnings using under both GNU and Intel compiler suites.  This requirement is checked by the
nightly regressions.  \draco\ will also compile under Mac XCode, but
this is not a requirement. \index{C++!Intel} \index{C++!Portland Group}

%%----------------------------------------
\subsection{Build tools}

For \draco\ to be correctly configured you must use \cmake\ and \index{cmake}
\ctest~\cite{cmake} version 2.8.8\footnote{\cmake\ version 2.8.8 introduced support for exported and imported build system targets used by \draco\ and client projects} or later.  Many machines at LANL
provide \cmake, but not 2.8.8+.  Thus, \cmake\ is provided as a part
...
access to these tools on platforms that do not provide them natively. \index{SVN} \index{Python} \index{numdiff}
...
\label{doxygen}
\index{Doxygen} \index{documentation!automatic} 
...
PDF for viewing.  If you want to build \LaTeX\ documentation from a Windows PC, we recommend the use of TeXworks' MikTeX.  For proper compilation of \LaTeX\ sources into PDF you may need to set the environment variables shown in Table~\ref{tab:latex_environ}. \index{TeX Works}
%
\begin{table}[!htbp]%
...
\index{instrumentation} \index{profiling}
...
\index{valgrind} \index{instrumentation!valgrind} \index{instrumentation!memory leaks}
...
Dynamic Analysis (i.e.: \textbf{valgrind}) results are provided by the  \index{instrumentation!dynamic analysis}
...
\index{instrumentation!code coverage analysis} \index{Bullseye Code Coverage}
...
\index{instrumentation!lines-of-code analysis} \index{CLOC}
...
\url{consult@lanl.gov}. \index{LANL HPC consultants}
...
have the \comp{SVN\_EDITOR} environment variable set in your shell % as shown in Listing~\ref{lst:setsvneditor}.  
If you
...
preferred editor(e.g. \comp{setenv SVN\_EDITOR emacs}).  Most \draco\ developers use \emacs\ and it is the
recommended editor for new users because \draco\ specific
functionality has been added to this editor~\cite{xtm:9909}.  If you do not set this environment variable, Linux systems will assume \comp{vi}.  You can also choose another editor such as \soft{NEdit}.
...
the recommended \draco\ developer environment setup.  These changes should
...
Ref.~\ref{chap:model}.  We will not repeat that discussion here.
\draco\ and its clients will check out
of the \svn\ repository into a \emph{source} directory (usually in your
personal workspace).  You will also create one or more \emph{build}
directories where configuration and compiling will take place and unit \index{build directory}
...
command. \index{SVN:checkout}

\footnotesize
\begin{verse}
\texttt{[/var/tmp/work/source] \% svn co svn+ssh://ccscs8/ccs/codes/radtran/svn/draco/trunk draco \\
\emph{U    draco/environment/bashrc/.bashrc} \\
\emph{U    draco/environment/bashrc/.bashrc\_linux64} \\
\emph{U    draco/environment/bashrc/.bashrc\_linux} \\
\ldots}
\end{verse}
\normalsize
...
\begin{verse}
\texttt{[/var/tmp/work] \% ls -F \\
\emph{source/\ \ \ linux\_mpid/\ \ linux\_scalar/ }}
\end{verse}
...
\begin{verse}
\texttt{[/var/tmp/work] \% ls -F source \\
\emph{draco/  capsaicin/}}
\end{verse}
\normalsize

We will configure the sources in these directories.  \comp{cmake}
...
directory will be created automatically by the \draco\ make system. \index{target directory} \index{prefix directory}
...
generated build project. \index{build project} \index{Makefile} \index{XCode} \index{Eclipse CDT}
...
\texttt{[/var/tmp/work/linux\_mpid/draco] \% cmake \\
  \hspace{0.5in}-DCMAKE\_INSTALL\_PREFIX=/var/tmp/work/linux\_mpid
  /var/tmp/work/draco} \\
...
the \draco\ development environment's use of vendor modules loaded automatically by the \comp{.bashrc} provided in \draco's \comp{environment} directory.
...
\normalsize
%
Alternatively, you can load the \draco\ module for \comp{XMGrace} that will set these values for you automatically.
...
\texttt{[/var/tmp/work/linux\_mpid/draco] \% cmake \\
  ...
  \hspace{0.5in}/var/tmp/work/source/draco} \\
\end{verse}
\normalsize

Once the configuration is complete the files required for building the
\draco\ libraries will exist in the binary directory.  The Makefiles
...
simply run \textbf{gmake} to create the \draco\ libraries.  You may want
...
process.  We recommend that \textbf{N} be set to value that is about 50\% larger than the number of available cores (i.e.: 24 on a 16 core machine).

\footnotesize
\begin{verse}
\texttt{[/var/tmp/work/linux\_mpid/draco] \% gmake -j 24}
\end{verse}
\normalsize
%
Once the build process has finished several new directories will exist
in the binary directory.    If the \comp{install} target is requested (e.g.: \comp{make -j24 install}), the \draco\ library files will have been
installed in the \emph{prefix} directory (e.g.: \comp{/var/tmp/work/linux\_mpid/lib}).  The C++ header
...
\label{regression} \index{regression tests} \index{ctest}
...
\comp{ctest} from the \draco\ binary directory
(\url{/var/tmp/work/linux_mpid/draco}).  Similarly you can run the
same command from the \capsaicin\ binary directory (or any other
client's binary directory) to run all of the \capsaicin\ (client's)
regression tests.
...
\S\ref{doxygen}~\cite{doxygen}).  Once you are setup to use \doxygen\ and the
...
command from the \draco\ build directory.

\footnotesize
\begin{verbatim}
     [...rk/linux_mpid/draco] % make autodoc
\end{verbatim}
\normalsize

This will create two new directories under
\comp{/var/tmp/work/\emph{targetdir}/autodoc} named \comp{html}.
...
If you encounter a C++ error message that you don't understand, you should contact other \draco\ developers and search the web for help.  The following web sites may be of some use:
\begin{itemize}
