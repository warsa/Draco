...
precluded in \draco.  In particular, \draco\ has plans to support \lang{ISO\_C\_BINDING} \index{Fortran!ISO\_C\_BINDING} interfacing between \cpp\ and \fortran \index{Fortran!F90}.  A more general type of 
...
and the generic concept-model \index{generic concept-model} idea.  Other software engineering
...
standard and made use of  \autoconf~\cite{autoconf}, \index{autoconf}
\gmake~\cite{gmake} \index{make}, and \gmfour~\cite{m4} \index{m4} for configuring and building components.  
In 2011, this requirement was altered as the development team chose to replace the
aging build system with a \cmake-based system~\cite{cmake}. \index{cmake}  In addition, \draco\ version
...
tracked using \teamforge~\cite{teamforge,teamforge-lanl}. \index{TeamForge} In addition, an archived email
list is available to submit design plans and discussion between team
members.  Also, \valgrind~\cite{valgrind}, \index{valgrind} \bullseye~\cite{bullseyeweb} \index{Bullseye Coverage} and \cloc~\cite{clocweb} \index{CLOC} play important roles in the
...
Standard \cpp~\cite{ansi:cpp} \index{C++!1998 ANSI Standard}.  \draco\ does not currently support the 2011 ANSI \cpp\ Standard~\cite{ansi:cpp11}\index{C++! 2011 ANSI Standard} because LANL standard compilers do not support the new standard yet.  As soon as the required compilers support the new standard, \draco\ will allow code to be written that conforms to the 2011 standard. Currently, any \cpp\ compiler that conforms to the 1998 C++ standard will compile \draco.  Portland Group, Intel and GNU \cpp\ are regularly used for building on ASC hardware. 
Currently, there
...
languages to be located in a directory that is included in the developer's \comp{PATH},
...
(in particular \lang{Fortran} \index{Fortran}) to be supported for interfacing and numerical
