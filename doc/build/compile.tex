...
\begin{figure}
  \centerline{\includegraphics[clip,trim=1cm 4cm 0cm 4cm, width=7.5in, angle=90]{fig/level}} % brtl
...
When linking \draco\ into an external product, all component dependent
libraries must be included.\index{dependencies!component}  The list of necessary libraries for
linking a package is given in Table~\ref{tab:depends}.  This list of dependencies is also captured in the installed file \comp{lib/cmake/draco-6.4/draco-config.cmake}. In summary,
\begin{table}
  \caption{
...
%       \pkg{mesh} \\ 
       ...
       \pkg{special\_functions} & 4 & \pkg{roots}, \pkg{ode}, \pkg{units} & \pkg{linear}, \dsxx & - \\
       \pkg{cdi\_analytic} & 4 & \pkg{parser}, \pkg{ode}, \pkg{cdi} & \cfour, \dsxx, \pkg{units} & - \\

       \pkg{quadrature} & 5 & \pkg{parser},                 & \cfour, \dsxx, \pkg{units},  & - \\
                                       &  & \pkg{mesh\_element},  & \pkg{ode}, \pkg{roots} &  \\
                                       &  & \pkg{special\_functions} & \pkg{linear} &  \\
...
\draco\ requires support (vendor) libraries\index{dependencies!vendor} in order to build specific components (e.g.: \pkg{c4} requires \soft{MPI}).  Additionally, some optional features in \draco\ can only be enabled if optional external vendor libraries are available to the local build (e.g.: \pkg{lapack\_wrap} needs \soft{LAPACK}).  Table~\ref{tab:vendor} lists all of the present
\begin{table}
...
components.  Because \draco\ is based on the concepts of levelized
...
Table~\ref{tab:depends} we see that \pkg{quadrature} is directly dependent on the \draco\ 
packages \pkg{special\_functions}, \pkg{parser} and \pkg{mesh\_element}.  Its implicitly dependent on \pkg{ode}, \dsxx, \pkg{units}, \pkg{roots}, \pkg{linear} and \cfour.  From Table~\ref{tab:vendor} we see
...
do require these libraries.  In practice, the \draco\ build system attempts to find and use full paths to vendor and \draco\ component libraries so that \ldlib\ does not need to be manipulated by the developer or software user.  The build system also locates and sets all of the libraries required or each vendor so that a \draco\ component or client only needs to specify that it should link to a set of libraries provided by the \cmake\ variable  \$\vble{\{\$\{VENDOR\}}\comp{\_LIBRARIES\}}.  We note that the installed file \comp{lib/cmake/draco-6.4/draco-config.cmake} captures all of the interdependency and vendor dependencies shown here.  This file is a list of \latin{exported} \cmake\ targets for \draco\ and can be used by \cmake\ to \latin{import} all of the \draco\ dependencies and targets into another build system.
...
\item \pkg{Random123} provides random numbers, see \S~\ref{appsec:random123};
...
%      & ENV\{LD\_LIBRARY\_PATH\} & To help the build system find \pkg{LAPACK}, ensure that the library location is appended to this environment variable. \\ 
       \pkg{LAPACK}$^{\text{b}}$ & ENV\{LAPACK\_LIB\_DIR\} & To load a specific \pkg{LAPACK}, ensure this variable is set to the desired location. \\
      \hline
% GSL
      ...
      \hline
% EOSPAC
      \pkg{EOSPAC} & ENV\{EOSPAC\_INC\_DIR\} & Help the build system find the desired installation by setting this environment variable. \\
      &  ENV\{EOSPAC\_LIB\_DIR\} & Help the build system find the desired installation by setting this environment variable. \\
      \hline
% Random123
      \pkg{Random123} & ENV\{RANDOM123\_INC\_DIR\} & Help the build system find the desired installation by setting this environment variable. \\
      \hline
% XMGRACe
...
%      & --with-pcglib-lib \\
      ...
      \multicolumn{3}{p{6in}}{$^{\text{b}}${\footnotesize As of \draco-6\_5\_0, the \draco\ build system imports the \pkg{LAPACK} targets via the \cmake\ imported targets feature.}} \\
...
\begin{verbatim}
     $ cmake -DCMAKE_TOOLCHAIN_FILE=$draco_home/config/Toolchain-roadrunner-spu.cmake \
             $draco_home
\end{verbatim}
