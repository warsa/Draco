...
features (\texttt{FindMPI.cmake}) only found in \cmake-2.8.5 or
newer. 

\cmake\ is has mature cross platform support, can construct build
projects for various systems (Makefiles, XCode, Eclipse, Visual
Studio, etc.).  \cmake\ also has an excellent dependency generator for
both source code and tests allowing multi-threaded builds (e.g.:
\texttt{make -j16} to make the most of available hardware.
...
started as a stand alone GUI (e.g.: \texttt{cmake-gui}).
...
\label{sec:dracodash}
\draco's regression system has been replaced with the use of
\ctest\ and \cdash.  Nightly regression ctest scripts post results to
the \draco\ dashboard, which can be viewed by pointing your web
browswer at \texttt{http://coder.lanl.gov/cdash}.   Visit the web site to
create an account if you want email notification from the regression
system.  If you have trouble with the \cdash\ web page, contact Kelly
Thompson, \texttt{kgt@lanl.gov}, for assistance.
...
\texttt{ccscs8} also reported code coverage statistics, lines-of-code
statistics and a valgrind dynamic analysis report.  A screenshot of the
dashboard is shown in Fig.~\ref{fig:dashboard}.
...
\item Eliminate component level release information and files.  All
  components will point to the general Relase information found in the
  \textsf{ds++} component.  \draco\ version numbers are now derived
  from the \cmake\ build system instead of being hard coded.
\item When using the \cmake\ GUI or \texttt{ccmake} some values are
  now contrained to permittable values.
\item Tweaks to test registration to improve how tests are run in a
...
\item Provide cross-compiling support to support building \draco\ on
...
\label{changes:c4}
\begin{itemize}
\item Redesign the \texttt{ApplicationUnitTest} class so that it uses
  the CPP macro \texttt{DRACO\_UNAME} instead of the deprecated
  \texttt{C4\_UNAME}. Change the CPP macros of the form
  \texttt{c4\_is<platform>} to \texttt{draco\_is<platform>}. Finally,
  the define for \texttt{C4\_MPICMD} has been moved to
  \texttt{c4/config.h} and away from \texttt{ApplicationUnitTest.hh}.
\item Use the new functions from \texttt{ds++/path.hh} instead of
  redefining our own in \textsf{c4}.
\item Update the \texttt{ApplicationUnitTest} class so that it works
  on Cielito (i.e.: teach it to run \texttt{aprun}).
\item \texttt{C4\_Functions.hh} defines a new function
  \texttt{send\_is} that will invoke \texttt{MPI\_Issend}.
\item Update \texttt{ParallelUnitTest} to correct a deficiency that
  allowed the test to report \textsf{PASS} when a failure only occured
  on a non-IO processor.  Also eliminated a potential race condition
  found in the \texttt{status()} member function.

\item Update \texttt{Send\_Receive} so that the sender now holds onto
  size in a private member variable.  This prevents the value from
  going out of scope before the send completes.  \\ 
\\
...
\item Retire \texttt{Release.hh/cc} and point the code to
  \texttt{ds++/Release.hh/cc}.
\item General improvements to eliminate compiler warnings.
\item Retire SCons scripts.
...
\label{changes:cdi}
\begin{itemize}
\item In \texttt{OpacityCommon.hh}, provide a new enum value for
  \texttt{rtt\_cdi::Reaction} to indicate the end of the enumeration
  list.  This is uselful for default initialization.
...
\item Rename \texttt{Analytic\_Multigroup\_Opacity} to
...
\item Provide a new utility,\texttt{GandolfInterpreter}, that can read
  IPCRESS files and interactively provide information about its
  content~\cite{gandolf,tops}.  An example use command is:
%
\begin{lstlisting}[basicstyle=\footnotesize,
    xleftmargin=1.0in, xrightmargin=1.0in]
...
\item Retire \texttt{DynArray}.
\item Update \texttt{Mat} classes to use STL allocators instead of
  \texttt{ds++/Allocators}, which are being retired.
\item Provide a new function \texttt{verbose\_error()} in
  \texttt{path.hh}. 
\item Refactor \texttt{UnitTest} by moving some file/path manipulation
  logic into \texttt{ds++/path.hh}.
\item Update \texttt{path} by providing the member functions
  \texttt{fileExists()} and \texttt{getFilenameComponent()}. 

\item Provide \texttt{swap.hh}.  This class provides functions that do
  byte-swapping in one of two ways: either use GNU extended asm for
...
\begin{itemize}
\item The build system will omit \texttt{lapack\_wrap} from the build
  if \textsf{LAPACK} is not found.
\item Retire \texttt{Release.hh/cc} and point the code to
  \texttt{ds++/Release.hh/cc}.  With this change,
  \texttt{lapack\_wrap} is now a header-only component and no library
  will be created or installed.
\item Retire SCons scripts.
\end{itemize}
...
\item Updated \texttt{Ordinate} so that the ordinates can be sorted
  based on a comparator supplied to the \texttt{QuadServices}
...
\item Add an exponential integral evaluation function,
...
\begin{itemize}
\item Refactor some code and the unit tests to improve code coverage.
  In particular, capture some of the output that was going to
  \texttt{cout} and examine it from within the tests.
\item Retire \texttt{Release.hh/cc} and point the code to
  \texttt{ds++/Release.hh/cc}.  
\item Retire SCons scripts.
\end{itemize}
...
\item Bugs associated with the old autoconf-based build system are
...
shows a slight improvement in the function point coverage (~2\%) when
compared to \draco-6\_0\_0.
\input{cov-6_2_0}

The \cdash\ \hyperref[http://coder.lanl.gov/cdash]{dashboard} for
\draco\ prsents all of these code statistics and also valgrind results
for nightly regression builds.  The data is presented in a more
interactive format that is more useful for day-to-day development.
...
\begin{center}
  ...
  \begin{tabular}{lp{2.5in}}
    ...
    \textsf{RoadRunner} & \texttt{gcc/gcc-ppu/gcc-spu}  \\
...
Additional build directories named \texttt{debug\_verbose} may also
be found on some platforms.  These additional builds are typically
special debug version used by the Jayenne codes.  Common names
include: \texttt{debug\_nodbc}, \texttt{opt\_log},
\texttt{debug\_nodbc\_nr}, \texttt{opt\_nr}, \texttt{opt\_nolog\_nr},
and \texttt{debug\_nolog}.
