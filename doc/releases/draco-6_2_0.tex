...
% generate file list
% cd draco
% files=`find . -name '*.hh' -o -name '*.cc' -o -name '*.txt' -o -name '*.cmake' -o -name '*.in' -o -name '*.h' -o -name '*.tex'`
% cvs annotate $files > blame

% generate user list
% cat blame | awk '{print $2}' > blame.ulist
% ulist=`cat blame.ulist | sort -u  | sed -e 's/^(//'`

% generate report
% for name in $ulist; do numlines=`grep $name blame | wc -l`; echo \
% "$numlines: $name"; done > blame_count
% cat blame_count | sort -rn
