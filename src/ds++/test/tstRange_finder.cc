...
//     if (index != -1) ut.failure("test FAILS");
    ...
    if (index != 5) ut.failure("test FAILS");
...
//     if (index != -1) ut.failure("test FAILS");
    ...
    if (validate(std::pair<double*, double*>(rv+0, rv+0), rv+0, rv+10))
    {
    ...
    if (validate(std::pair<double*, double*>(rv+10, rv+10), rv+0, rv+10))
    {
        ut.failure("validate FAILED to catch out of range result");
    }
    else
...
void test_range_finder_right(UnitTest &ut)
{
	...
	if (index != i-1) ut.failure("test FAILS");
    }
