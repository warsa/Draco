...
bool check_4(Ordinate const &ordinate)
{
    ...
    // two octants.
    if (ordinate.eta()<0 || ordinate.xi()<0) return false;
    return true;
}
...
bool check_2(Ordinate const &ordinate)
{
    ...
    // four octants
    if (ordinate.xi()<0) return false;
    return true;
}

//---------------------------------------------------------------------------//
typedef std::pair<double, size_t> dsp;
bool bigger_pair(const dsp& d1, const dsp& d2)
{
 ...
 */
void
Ordinate_Set_Mapper::map_angle_into_ordinates(const Ordinate& ord_in,
    ...
    const
{
    ...
    switch (interp_in)
    {
        ...
        case NEAREST_THREE:
        {
            ...
            size_t i(0);
            for ( auto elem=dpsi.begin(); elem != dpsi.end();
                 ++elem, ++i)
            {
                elem->first = dps[i];
                elem->second = i;
            ...
            std::nth_element(dpsi.begin(), dpsi.begin()+3,
                             dpsi.end(), bigger_pair);
