...
// -----------------------------------------------------------------------------
double zeroth_moment(const vector<double> &weights,
                     const Ordinate_Set& os)
{
                            ...
                            const size_t index)
{
    ...
    if ( !soft_equiv(E, ord.wt())) ut.failure("Weight summation mismatch");
    ...
    if ( max_e != nz_e ) ut.failure("Nearest ordinate mismatch");
    ...
    if ( nz_e != index ) ut.failure("wrong ordinate mapped");
}
                         ...
                         const vector<double>& wts)
{
    ...
    // weight vector
    if ( wts[i1] < wts[i2]) ut.failure("Max weight is smaller than 2nd weight");
    if ( wts[i1] < wts[i3]) ut.failure("Max weight is smaller than 3rd weight");
...
void ordinate_set_2D_nn_mapper_test( rtt_dsxx::UnitTest &ut)
{
    ...
    SP<Ordinate_Set> os_LS2 = quadrature.create_ordinate_set(2,
                                                   geometry,
                                                   1.0, // norm,
...
void ordinate_set_1D_nn_mapper_test( rtt_dsxx::UnitTest &ut)
{
    ...
    SP<Ordinate_Set> os_LS4 = quadrature.create_ordinate_set(1, //1-D
                                                 geometry,
                                                 1.0, // norm,
                                                 false, //starting_directions?
                                                 false, //extra directions?
...
void ordinate_set_3D_nn_mapper_test( rtt_dsxx::UnitTest &ut)
{
    ...
    SP<Ordinate_Set> os_LS2 = quadrature.create_ordinate_set(3,
                                             geometry,
                                             1.0, // norm,
                                             false, //starting_directions?
                                             false, //extra directions?
