...
def generate_config_h( target, source, env):
    ...
    generate the config.h file for cdi_gandolf.
    ...
    ## Example use:
    build_w_gandolf = config['build_w_gandolf']
    if build_w_gandolf == False:
        outtext = outtext.replace("<gandolf stub>",
                                  "#define rtt_cdi_gandolf_stub 1")
    else:
        outtext = outtext.replace("<gandolf stub>",
                                  "// #define rtt_cdi_gandolf_stub 1")
