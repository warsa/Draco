...
void cartesian_mesh_2d(rtt_c4::ParallelUnitTest &ut) {
  ...
  if (mesh->get_cell_type() != cell_type)
    ITFAILS;
  if (mesh->get_cell_to_node_linkage() != cell_to_node_linkage)
    ITFAILS;

  // check that flat side type and side to node linkage is correct
  if (mesh->get_side_node_count() != side_node_count)
    ITFAILS;
  if (mesh->get_side_to_node_linkage() != side_to_node_linkage)
    ITFAILS;
  ...
  if (layout.size() != mesh_iface.num_cells)
    ITFAILS;
