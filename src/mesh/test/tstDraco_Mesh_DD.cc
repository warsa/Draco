...
void cartesian_mesh_2d_dd(rtt_c4::ParallelUnitTest &ut) {
  ...
  std::vector<unsigned> ghost_cell_number = {0};
  std::vector<unsigned> ghost_cell_to_node_linkage(2);
  std::vector<unsigned> ghost_cell_rank(1);
  if (rtt_c4::node() == 0) {
  ...
  if (mesh->get_geometry() != Draco_Mesh::Geometry::CARTESIAN)
    ITFAILS;
  if (mesh->get_num_cells() != mesh_iface->num_cells)
    ITFAILS;
  if (mesh->get_num_nodes() != mesh_iface->num_nodes)
    ITFAILS;

  // check that the vector data is correct
  if (mesh->get_ghost_cell_numbers() != ghost_cell_number)
    ITFAILS;
  if (mesh->get_ghost_cell_ranks() != ghost_cell_rank)
    ITFAILS;
  ...
  if (bd_layout.size() != mesh_iface->num_cells)
    ITFAILS;
  ...
  if (go_layout.size() != mesh_iface->num_cells) {
    std::cout << "go_layout.size() = " << go_layout.size() << std::endl;
    ITFAILS;
  }
  ...
  // check that ghost-cell-to-node linkage data is correct
  {
    ...
    for (unsigned ghost = 0; ghost < num_ghost_cells; ++ghost) {
      ...
      if (!std::is_permutation(test_gn_first,
                               test_gn_first + ghost_cell_type[ghost], gn_first,
