...
class And_Expression : public Expression
{
  ...
  private:
    ...
    {
        return
            evaluate_def_(e1_, x) && evaluate_def_(e2_, x);
    }
    ...
    virtual void write_(Precedence const p,
...
class Cos_Expression : public Expression
{
  public:
    Cos_Expression(pE const &expression)
        :
        Expression(expression->number_of_variables(),
                   dimensionless),
...
class Power_Expression : public Expression
{
  public:
    Power_Expression(pE const &e1, pE const &e2)
        ...
        Expression(e1->number_of_variables(),
                   pow(e1->units(),
                       (*e2)(vector<double>(e2->number_of_variables(),
                                            0.0)))),
  ...
  private:
    ...
    {
        return
            std::pow(evaluate_def_(e1_, x), evaluate_def_(e2_, x));
    }
...
{
  public:
    Variable_Expression(unsigned const index,
    ...
    bool check_class_invariant() const
    {
        return index_<number_of_variables();
    }
  ...
  private:
    ...
    virtual void write_(Precedence, vector<string> const &vars, ostream &out) const
    {
                        ...
                        Token_Stream &tokens)
{
    if (at_real(tokens))
    {
        return pE(new Constant_Expression(number_of_variables,
                                          parse_real(tokens)));
    }
    else if (tokens.lookahead().text()=="(")
    {
        tokens.shift();
        pE Result = Expression::parse(number_of_variables,
                                      variable_map,
                                      tokens);
    ...
    else
    {
            ...
            // a function call
        {
            ...
            pE const argument = parse_primary(number_of_variables,
                                              variable_map,
                                              tokens);
            switch (name[0])
            {
            ...
            // a variable or constant name
        {
            ...
            {
                return pE(new Variable_Expression(i->second.first,
                                                  number_of_variables,
                                                  (unit_expressions_are_required()?
                                                   i->second.second : dimensionless)));
            }
            else
            {
                ...
                map<string, Unit>::const_iterator i =
                    unit_map.find(variable.text());
                      ...
                      Token_Stream &tokens)
{
    ...
    {
        tokens.shift();
        return pE(new Negate_Expression(parse_unary(number_of_variables,
                                                    variable_map,
                                                    tokens)));
    }
    else if (tokens.lookahead().text()=="!")
    {
        tokens.shift();
        return pE(new Not_Expression(parse_unary(number_of_variables,
                                                 variable_map,
                                                 tokens)));
    }
    else
...
static pE parse_additive(unsigned const number_of_variables,
                         ...
                         Token_Stream &tokens)
{
    pE Result = parse_multiplicative(number_of_variables,
                                     variable_map,
                                     tokens);
    while (tokens.lookahead().text()=="+" ||
           tokens.lookahead().text()=="-")
    {
        ...
        {
            tokens.shift();
            pE const Right = parse_multiplicative(number_of_variables,
                                                  variable_map,
                                                  tokens);
            if (!is_compatible(Result->units(),
                               Right->units()))
            {
                           ...
                           Token_Stream &tokens)
{
           ...
           token.text()=="<=" || token.text()==">=")
    {
        ...
        {
            tokens.shift();
            pE const Right = parse_additive(number_of_variables,
                                            variable_map,
                                            tokens);
            if (!is_compatible(Result->units(),
                               Right->units()))
...
//---------------------------------------------------------------------------//
void Expression::write(Precedence const p,
                       vector<string> const &vars,
                       ostream &out) const
{
