 ...
 * \return Value of the relativistic Fermi-Dirac integral for the given
 * chemical potential.
 */
static double Feta_diff(double eta)
{
    return F_eta(eta, lgamma) - ln;
}
 ...
 * 
 * \param n
 * Dimensionless number density \f$\frac{\partial
 ...
 *
 * \note This implementation is very expensive.  it is also of limited
 * utility, since for problems for which the relativistic form of the
 * Fermi-Dirac function is needed, pair production is likely to be important,
...
double F_eta_inv(double const n, double const gamma)
{
    ...
    if (n==0) return -numeric_limits<double>::max();
    ...
    double x1 = eta*0.99, x2 = 1.01*eta;
    if (x2<x1) swap(x1, x2);
