...
double F_eta(double const eta, double const gamma)
{
	...
	if (x<0.07)
	{
	    n1 = 0.2*x*x*x*x*x*(1 - (5./14.)*x*x + (5./24.)*x*x*x*x
		- (25./176.)*x*x*x*x*x*x + (35./1024.)*x*x*x*x*x*x*x*x);
	    dn1 = dx*(5*n1/x + 0.2*x*x*x*x*x*x*(-5./7. + (5./6.)*x*x 
						-(75./88.)*x*x*x*x + 
						(35./128.)*x*x*x*x*x*x));
	}   
	else
	...
	else
	{
		...
		max(max1, max(max2, max3)) * (max(eta,0.0)+gamma);
	    ...
	    typedef void(*fpv)(std::vector<double> &,
	    ...
	    return quad( &Feta_integrand,
			 0.0,
			 max(eta,0.0)+50*gamma,
			 tol,
			 rkqs_fpv );
	}
    }
