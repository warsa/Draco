 ...
 * orthonormal matrix and \f$R\f$ is upper triangular.  This procedure
 * is specialized for a square matrix and does no pivoting (which is
 * necessary only if the matrix is very close to singular.)  The QR
 * decomposition is about twice as expensive as the LU decomposition and
 * so should be used only when the special form of the decomposition is
 * advantageous.
 *
 * \arg \a RandomContainer A random access container
 *
 * \param a
 * \f$N\times N\f$ matrix to be decomposed.  On exit, contains the
 * Householder vectors \f$u\f$ in its lower triangular part and the 
 * strict upper triangular part of \f$R\f$ in its strict upper triangular
 * part.
 * 
 * \param n Rank of the matrix \f$N\f$
 * 
 * \param c
 * On exit, contains the normalization \f$\frac{1}{2} u\cdot u\f$ of the 
 * Householder vectors \f$u\f$ .
 *
 * \param d
 * On exit, contains the diagonal part of the \f$R\f$ matrix.
 *
