...
//---------------------------------------------------------------------------//
    ...
    if ( !iam )
    {
        ...
        std::cout << "\nProcess grid: " << static_cast<int>(grid.nprow)
                  << " X " << static_cast<int>(grid.npcol) << "\n" << std::endl;
    }
...
//---------------------------------------------------------------------------//
     ...
     if( A.Stype != SLU_NR_loc ) ITFAILS;
     if( A.Dtype != SLU_D )      ITFAILS;
     if( A.Mtype != SLU_GE )     ITFAILS;
     if( A.nrow != 4960 )        ITFAILS;
     if( A.ncol != 4960 )        ITFAILS;
...
//---------------------------------------------------------------------------//
    ...
    */
    ...
    if( options.Fact != DOFACT )             ITFAILS;
    if( options.Equil != YES )               ITFAILS;
    if( options.ColPerm != METIS_AT_PLUS_A ) ITFAILS;
    if( options.IterRefine != SLU_DOUBLE )   ITFAILS;
    if( options.RowPerm != LargeDiag )       ITFAILS;
    ...
    if( ScalePermstruct.DiagScale != NOEQUIL ) ITFAILS;

    // Initialize the statistics variables.
    PStatInit(&stat);

    if( *stat.ops != 0 )       ITFAILS;
    if( stat.TinyPivots != 0 ) ITFAILS;

    // Call the linear equation solver.
    pdgssvx(&options, &A, &ScalePermstruct, b, ldb, nrhs, &grid,
            &LUstruct, &SOLVEstruct, berr, &stat, &info);

    if( ! rtt_dsxx::soft_equiv(*berr,0.0) ) ITFAILS;
    if( info != 0 )                         ITFAILS;
    if( options.SolveInitialized != YES )   ITFAILS;
    if( options.RefineInitialized != YES )  ITFAILS;
    if( ScalePermstruct.DiagScale != BOTH)  ITFAILS;
