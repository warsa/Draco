...
void RTT_Mesh_Reader::transform2CGNS(void)
{
    ...
    std::vector< std::vector< std::vector< size_t > > > cell_ordered_sides(rttMesh->get_dims_ncell_defs());
	...
	unique_element_types.push_back(cell_def);
        ...
        if (cell_def == Element_Definition::POLYGON)
        {
            ...
            cell.reset(new rtt_mesh_element::Element_Definition(cell_definition->get_name(),
        ...
        else if (cell_def == Element_Definition::POLYHEDRON)
        {
            ...
            for (unsigned s=0; s<cell_definition->get_nsides(); ++s)
            {
                ...
                std::vector<int>::iterator sit(std::find(side_types.begin(),side_types.end(),side_type));
            ...
            if (have_quad9)
            {
                Insist(have_quad4, "quad or quad4 must appear in the cell_defs block when a quad9 is in a polyhedron");
            }
            ...
            for (unsigned s=0; s<cell_definition->get_nsides(); ++s)
            {
                ...
                if (sit == side_types.end()) 
                {
                    // not yet added for this polyhedron, so create an element of this type
                    Element_Definition elem_def(*cell_definitions[side_type]);
