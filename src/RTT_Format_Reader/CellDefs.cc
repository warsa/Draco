    ...
    std::vector<std::vector<std::vector<size_t>>> const & cell_ordered_sides)
{
    ...
    for (size_t cd = 0; cd < static_cast<size_t>(dims.get_ncell_defs()); cd++)
    {
        ...
        defs[cd]->redefineCellDef(
            cell_side_types[cd], cell_ordered_sides[cd], dims.get_ndim());
    }
}
/*!
...
void CellDef::readDef(ifstream & meshfile)
{
    ...
    if (nsides > 0) std::getline(meshfile, dummyString);
    ...
    for (unsigned i = 0; i < nsides; ++i)
    {
        ...
        if (sides[i].size() > 0) std::getline(meshfile, dummyString);
    }
}
/*!
 * \brief Changes the cell definitions specified in the RTT_Format file
    ...
    int const ndim)
{
    ...
    else if (name == "tetrahedron")
    {
        ...
        while (std::count(new_ordered_sides[new_side].begin(),
                          new_ordered_sides[new_side].end(),
                          node_map[0]) > 0)
        {
            ++new_side;
            Insist(new_side < nsides,
    ...
    else if (name == "hexahedron")
    {
        ...
        for (size_t n = 0; n < ordered_sides[quad].size(); n++)
        {
            ...
            {
                if (new_node_count[c] == 0) new_node = c;
                if (old_node_count[c] == 0) old_node = c;
            }
            node_map[old_node] = new_node;
